...
{
    public class RealmFactoryNativeFileReader : FileReader<Project>
    {
        ...
        private Level ParseLevelNode(XmlNode levelNode, Project project, Dictionary<int, object> objectIdDictionary)
        {
            ...
            foreach (XmlNode child in levelNode.ChildNodes)
            {
                ...
                {
                    foreach (XmlNode objectNode in child.ChildNodes)
                    {
                        ParseObjectNode(objectNode, level, objectIdDictionary);
                    }
                }
            }
        ...
        private void ParseObjectNode(XmlNode objectNode, Level level, Dictionary<int, object> objectIdDictionary)
        {
            int row = Convert.ToInt32(objectNode.Attributes["row"].Value);
            int column = Convert.ToInt32(objectNode.Attributes["column"].Value);
            int type = Convert.ToInt32(objectNode.Attributes["type"].Value);

            GameObjectType gameObjectType = (GameObjectType)objectIdDictionary[type];
            GameObject gameObject = gameObjectType.GenerateNew();

            level.Put(gameObject, column, row);
        }
