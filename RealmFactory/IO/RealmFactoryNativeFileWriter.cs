...
{
    public class RealmFactoryNativeFileWriter : FileWriter<Project>
    {
        ...
        private void WriteLevel(XmlWriter xmlWriter, Level level, Dictionary<object, int> objectIdDictionary)
        {
            ...
            for (int row = 0; row < level.Settings.Rows; row++)
            {
                ...
                {
                    if(level.Get(column, row) != null)
                    {
                        xmlWriter.WriteStartElement("object");
                        xmlWriter.WriteAttributeString("row", row.ToString());
                        xmlWriter.WriteAttributeString("column", column.ToString());
                        xmlWriter.WriteAttributeString("type", objectIdDictionary[level.Get(column, row).ParentType].ToString());
